...
function NewReportModal({
  ...
  const handleSubmit = async (event) => {
    ...
    // Calculate Timestamp and Hour Index 
    const date = new Date();
    
    // If reporting for previous time, update the hours/minutes
    if (reportScope === "previous") {
        const [hours, minutes] = reportTime.split(":").map(Number);
        date.setHours(hours, minutes, 0, 0);
    }

    // Capture the local hour index (0-23) so the backend updates the correct bar
    ...
    const payload = {
      ...
      timestamp: timestamp, // Send the calculated time
      hour: hourIndex       // Send the specific hour to update
    };
        ...
        >
          ...
          <div className="flex items-start justify-between">
            ...
            <button
              ...
              className="inline-flex size-10 items-center justify-center rounded-full bg-gray-100 text-gray-500 transition hover:bg-gray-200 hover:text-gray-700 disabled:opacity-50"
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <Button
              type="submit"
              disabled={isSubmitting}
              className="rounded-xl bg-blue-600 px-6 py-2 text-sm font-semibold text-white shadow transition-all duration-200 ease-out hover:bg-blue-700 disabled:bg-blue-400"
            >
